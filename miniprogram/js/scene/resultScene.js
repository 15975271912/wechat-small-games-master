...
export default class ResultScene {
    ...
    drawEle () {
        ...
        this.againImg = Sprite.getImage('result-again');
        this.againSprite = new Sprite(this.againImg, (750 - this.againImg.width) / 2, this.rankSprite.y + this.rankSprite.height + 320,
            this.againImg.width, this.againImg.height);
        this.againSprite.draw(this.ctx);
    ...
    bindEvent () {
        ...
        wx.onTouchStart((e) => {
            ...
            if (x >= _this.rankSprite.x
                ...
                // 导出成绩单
                _this.report();
            }else if (x >= _this.againSprite.x
                && x <= _this.againSprite.x + _this.againSprite.width
                && y >= _this.againSprite.y
                && y <= _this.againSprite.y + _this.againSprite.height) {
                // 重新加载
                DataStore.getInstance().isGameOver = true;
                console.log(DataStore.getInstance().isGameOver);
                DataStore.getInstance().score = null;
                cancelAnimationFrame(_this.requestId);
